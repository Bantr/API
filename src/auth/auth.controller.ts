...
export class AuthController {
    ...
    steamReturn(@Req() req: Request, @Res() res: Response): void {
        const graphQLkey = this.authService.getGraphQLAuthKey(req.session.passport.user);
    ...
    async discordDisconnect(@Req() req: Request, @GetUser() user: User): Promise<void> {
    ...
    async faceitDisconnect(@Req() req: Request, @GetUser() user: User): Promise<void> {
    ...
    async graphQL(@Body() body: hasuraAuthDto): Promise<{ 'X-Hasura-User-Id': string; 'X-Hasura-Role': string }> {
        const graphQLkey = body.headers['bantr-graphql'];

        if (!graphQLkey) {
            throw new UnauthorizedException("You must provide a 'bantr-graphql' header.");
        }
